<template>
	<div class="container">
		<Navbar />
		<Nav :showactive="showactive" @datosUpdate="datosUpdate($event)" @netaUpdate="netaUpdate($event)" @linkUpdate="linkUpdate($event)" />
		<h1>{{ filename }}</h1>
		<ProcessedTable v-show="showactive[0]" :csvdata="csvdata" :tableConfig="tableConfig" />
		<TableByWell
			v-show="showactive[1]"
			:csvdata="csvdata"
		...
		/>
		<Matriz v-show="showactive[2]" :WellsUniqueCols="WellsUniqueCols" :WellsUniqueRows="WellsUniqueRows" :adjustedAverageBy4="adjustedAverageBy4" />
		<MatrizCero v-show="showactive[3]" :WellsUniqueCols="WellsUniqueCols" :WellsUniqueRows="WellsUniqueRows" :curveBy4="curveBy4" />
	</div>
</template>
...
export default {
	data() {
		return {
			...
			filename: [],
			tableConfig: [],
			adjustedHeader: ['Well', 'Lecture 1', 'Lecture 2', 'Lecture 3', 'Average'],
			Wells: [],
			groupByCSV: [],
			rawAdjusted: [],
			adjustedAverage: [],
	...
	created() {
		...
		if (this.$route.params.data != undefined) {
			...
			localStorage.setItem('adjustedAverage', JSON.stringify(this.adjustedAverage));
			/*** MATRICES ***/
		} else {
			...
			this.adjustedAverage = JSON.parse(localStorage.getItem('adjustedAverage'));
		}
	...
	beforeMount() {
		if (this.adjustedAverageBy4 != undefined) {
			localStorage.setItem('adjustedAverageBy4', JSON.stringify(this.adjustedAverageBy4));
		} else {
			this.adjustedAverageBy4 = JSON.parse(localStorage.getItem('adjustedAverageBy4'));
		}
